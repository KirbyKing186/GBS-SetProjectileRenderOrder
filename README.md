...
Although this engine plugin was built for GB Studio `4.0.2-e0`, it works on `4.1.2-e1` and should function with future GB Studio versions until `core.c`or `core.h` recieve an update. If either plugin is installed on a version that is not `4.0.2-e0`, the GB Studio compiler will return a warning. If you have verified the plugin works, update the version text within `engine/engine.json` to resolve the warning.

This plugin is currently updated for GB Studio `4.2.2`. For builds compatible with 4.1.3, try downloading the plugin from [this commit](https://github.com/KirbyKing186/GBS-SetProjectileRenderOrder/tree/85c4935c70bca4bd72caa111e6fe65a7ee2660da). If a future version of GB Studio incompatible with this plugin, try seeing what was modified in the new engine, and adapting those changes to the plugin. A [pull request](https://github.com/KirbyKing186/GBS-SetProjectileRenderOrder/pulls) would be highly appreciated if you are able to update it yourself. Otherwise, you can also make an [issue](https://github.com/KirbyKing186/GBS-SetProjectileRenderOrder/issues) and I will try and update it myself as soon as possible.
